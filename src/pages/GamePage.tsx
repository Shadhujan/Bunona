...
export function GamePage({ difficulty: propDifficulty }: GamePageProps) {
  const { difficulty: paramDifficulty } = useParams<{ difficulty: 'easy' | 'medium' | 'hard' }>();
      ...
      <div className="bg-white/90 rounded-lg border-2 border-yellow-500 p-8 w-full max-w-4xl">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-4">
            <div className="flex space-x-1" aria-label={`Lives remaining: ${lives}`}>
              {[...Array(3)].map((_, i) => (
                <Heart
                  key={i}
                  className={`w-6 h-6 ${
            ...
            <div className="text-lg font-semibold" aria-label={`Current score: ${score}`}>
              Score: {score}
            </div>
          </div>
          <div className="flex items-center space-x-2" aria-label={`Time remaining: ${timeLeft} seconds`}>
            <Timer className={`w-6 h-6 ${timeLeft <= 10 ? 'text-red-500' : 'text-gray-600'}`} />
            <span className={`font-mono text-xl ${timeLeft <= 10 ? 'text-red-500' : 'text-gray-600'}`}>
